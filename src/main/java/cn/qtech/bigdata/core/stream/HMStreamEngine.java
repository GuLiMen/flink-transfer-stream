...
import cn.qtech.bigdata.core.parser.HMParser;
import cn.qtech.bigdata.core.sink.HMKuduSink;
...
public class HMStreamEngine extends AStreamEngine {
    private static final Logger LOG = LoggerFactory.getLogger(HMStreamEngine.class);
    ...
    void initConifg() {
        try {
            PropertiesManager.loadProps("/HM-stream-srv.properties");
            LOG.info("==========加载配置文件===========");
        } catch (Exception e) {
    ...
    void srvConsumeQueueData() throws Exception {
        ...
        env.getCheckpointConfig().enableExternalizedCheckpoints(CheckpointConfig.ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION);
         env.setStateBackend(new FsStateBackend(new Path("hdfs://nameservice/flink/flink-checkpoints/HMStreamEngine")));
        env.setParallelism(parallelism);
        // 2.建立Kafka连接
        Properties properties = new Properties();
        properties.setProperty("bootstrap.servers", kafkaAddress);
        properties.setProperty("group.id", "HM_transfer");
        FlinkKafkaConsumer<String> consumer = new FlinkKafkaConsumer<>(topicName, new SimpleStringSchema(), properties);

        // 3.获取Kafka数据
          consumer.setStartFromGroupOffsets();
        env.addSource(consumer).uid("HMStreamEngineSource")
                .filter(x -> "HM".equals(JSON.parseObject(x).getString("device_type"))).uid("HMStreamEngineFilter")
                .map(new HMParser()).uid("HMStreamEngineMap")
                .addSink(new HMKuduSink()).uid("HMStreamEngineSink");

        env.execute(HMStreamEngine.class.getSimpleName());
    }
}
