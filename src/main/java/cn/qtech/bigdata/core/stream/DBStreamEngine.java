...
import cn.qtech.bigdata.core.parser.DBParser;
import cn.qtech.bigdata.core.sink.DBKuduSink;
...
public class DBStreamEngine extends AStreamEngine {
    private static final Logger LOG = LoggerFactory.getLogger(DBStreamEngine.class);
    ...
    void initConifg() {
        try {
            PropertiesManager.loadProps("/DB-stream-srv.properties");
            LOG.info("==========加载配置文件===========");
        } catch (Exception e) {
    ...
    void srvConsumeQueueData() throws Exception {
        ...
        env.setStateBackend(new FsStateBackend(new Path("hdfs://nameservice/flink/flink-checkpoints/DBStreamEngine")));
        env.setParallelism(parallelism);
        // 2.建立Kafka连接
        Properties properties = new Properties();
        properties.setProperty("bootstrap.servers", kafkaAddress);
        properties.setProperty("group.id", "DB_transfer");
        FlinkKafkaConsumer<String> consumer = new FlinkKafkaConsumer<>(topicName, new SimpleStringSchema(), properties);

        // 3.获取Kafka数据
        consumer.setStartFromGroupOffsets();
        env.addSource(consumer).uid("DBStreamEngineSource")
                .filter(x -> "Linda".equals(JSON.parseObject(x).getString("device_type"))).uid("DBStreamEngineFilter")
                .map(new DBParser()).uid("DBStreamEngineMap")
                .addSink(new DBKuduSink()).uid("DBStreamEngineSink");

        env.execute(DBStreamEngine.class.getSimpleName());
    }
}
