...
public class HMKuduSink extends RichSinkFunction<Map<String, String>> {
    ...
    private String tableName = AppConstants.KUDU_DATABASE + "." + AppConstants.HM_KUDU_TABLE;
    private static final Logger LOG = LoggerFactory.getLogger(HMKuduSink.class);
    ...
    public void invoke(Map<String, String> resultMap, Context context) throws Exception {
        if (resultMap == null) {
            LOG.error(" process err!! ==err count [" + (++AppConstants.errCounter) + "]==DBKuduWBSink.invoke() input map == null");
        ...
        if (deviceId == null || StringUtils.isBlank(deviceId) || create_date == null || StringUtils.isBlank(create_date)) {
            LOG.error(" DBKuduWBSink: device_id id is null,device_id:{} || create_date id is null,create_date:{}" + deviceId + create_date);
        ...
        try {
            ...
            String redisKey = AppConstants.HM_REDIS_KEY_PREFIX + deviceId;
            ...
            LOG.error(" 数据插入异常 ! HMKuduWBSink.invoke()", e);
        }
    ...
    private Boolean insertKudu(Map<String, String> resultMap) {
        ...
        try {
            ...
            LOG.error(" HMKuduWBSink.insertKudu() : insert failed !! ", e);
        }
        return false;
    ...
    public void close() throws Exception {
        ...
        try {
            ...
            LOG.error("关闭资源失败 ! HMKuduWBSink.close()", e);
        }
    }
}
